...
EOF
	...
	psql --dbname=$POSTGRES_DB --username=postgres --host=localhost -c 'CREATE TABLE IF NOT EXISTS user_roles (createdAt timestamp DEFAULT current_timestamp, updatedAt timestamp DEFAULT current_timestamp, roleId integer CONSTRAINT no_null NOT NULL PRIMARY KEY REFERENCES roles (id), userId integer CONSTRAINT no_null NOT NULL PRIMARY KEY REFERENCES users (id));'
